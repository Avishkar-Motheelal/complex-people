...
export class AccountService {
  ...
  register(user: User) {
    return this.http.post<HttpResponse<any>>(`${environment.apiUrl}/users`, user, {observe: 'response'});
  }
  ...
  LoginWithGoogle(credentials: string): Observable<HttpResponse<Object>> {
    const header = new HttpHeaders().set('Content-type', 'application/json');
    return this.http.post(`${environment.apiUrl}/login/oauth2/code/google`, JSON.stringify(credentials), {
      headers: header,
      observe: 'response'
    });
  }
